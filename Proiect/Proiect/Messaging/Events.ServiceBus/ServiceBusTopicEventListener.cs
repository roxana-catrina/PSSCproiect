...
namespace Proiect.Messaging.Events.ServiceBus;

/// <summary>
/// Implementation of IEventListener using Azure Service Bus
/// Listens to events from topics and dispatches them to registered handlers
/// </summary>
public class ServiceBusTopicEventListener : IEventListener
{
    ...
    private async Task ProcessMessageAsCloudEventAsync(ProcessMessageEventArgs arg)
    {
        ...
        CloudEvent cloudEvent = formatter.DecodeStructuredModeMessage(data.ToStream(), null, null);
    ...
    private async Task<EventProcessingResult> InvokeHandlerAsync(CloudEvent cloudEvent, IEventHandler handler)
    {
        try
        {
    ...
    {
        logger.LogInformation($"Event processing has succeeded {arg.Message.MessageId}");
